...
year_1 = list(years)[0]
year_2 = list(years)[1]
year1_index = year_1 - 1956
year2_index = year_2 - 1956

arg_list = []
fore_list = []
ratio_list = []

with open(agricultural_land_filename, encoding = 'utf8') as f:
    readcsv = csv.reader(f, delimiter = ',')
    for row in readcsv:
        if len(row) >=  (year2_index + 1):
            arg_list.append([row[0], row[year1_index], row[year2_index]])


with open(forest_filename, encoding = 'utf8') as f:
    readcsv = csv.reader(f, delimiter = ',')
    for row in readcsv:
        if len(row) >=  (year2_index + 1):
            fore_list.append([row[0], row[year1_index], row[year2_index]])

for i in range(1, len(arg_list)):
    try:
        if (float(arg_list[i][2]) - float(arg_list[i][1])) > 0 and (float(fore_list[i][2]) - float(fore_list[i][1])) > 0:
            ratio = (float(arg_list[i][2]) - float(arg_list[i][1])) / (float(fore_list[i][2]) - float(fore_list[i][1]))
            ratio_list.append([ratio, arg_list[i][0]])
    except ValueError:## float cant be null
        pass

ratio_list = sorted(ratio_list, key = lambda x : (-x[0], x[1]))

for i in range(len(ratio_list)):
    ratio_list[i][0] = format(ratio_list[i][0],'.2f')##!!!!! 强行保留2位小数,用round(x，2)如果第二位小数为零会省略

if len(ratio_list) < top_n:
    top_n = ratio_list
for i in range(top_n):
    countries.append(ratio_list[i][1]+' ('+str(ratio_list[i][0])+')')
